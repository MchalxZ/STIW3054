...
@RestController
public class WebhookController1 {

    private final ObjectMapper objectMapper;

    private static StringBuilder we = new StringBuilder();

    public WebhookController1(ObjectMapper objectMapper, CommentService commentService) {
        ...
        try {
            ...
            if ("created".equals(action)) {
                ...
                BotState w = chatStates.get(chatID);
                    messageProducer.sendMessage1("my-topic", WebhookController1.wee());
                if(w.equals(AWAITING_CONFIRMATION)){
                    ...
                    ww.start();
                    Thread.sleep(4000);
                    Thread ww2 = new Thread(new Runnable() {
                        @Override
                        public void run() {
                            SendMessage sendMessage = new SendMessage();
                            sendMessage.setChatId(chatID);
